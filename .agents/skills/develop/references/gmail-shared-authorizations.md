...
- Receive sync snapshots the method after entering `syncing`; both core and
  provider preflight failures may write `failed` only while the locked method
  still matches that binding, enabled state, phase, and lock version. Concurrent
  provider-setting rotation/removal or method disconnect therefore preserves the
  newer lifecycle and encrypted OAuth tokens.
