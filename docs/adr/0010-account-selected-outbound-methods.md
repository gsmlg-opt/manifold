...
running migration `20260811000100_add_shared_gmail_authorizations.exs`; old and
new code must not run concurrently. Start only the new release after all three
2026-08-11 migrations complete.

Migration `00100` moves legacy Gmail token material into the shared authorization
...
shape, or Gmail send methods exist. Migration `00200` refuses rollback when an
authorization event cannot map to exactly one legacy receive method. Migration
`00300` refuses rollback while any Gmail or SMTP provider submission exists.
